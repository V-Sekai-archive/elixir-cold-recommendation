...
defmodule RecGPT.AxonTrainTest do
  ...
  test "predict returns full-sequence logits (batch, seq_len, 15361)" do
    params = dummy_params()
    batch = 2
    seq_len = 4
  ...
  test "stream_batches yields {inputs, labels} with correct shapes" do
    ...
    stream = AxonTrain.stream_batches(seqs, token_id_list, item_embeddings, batch_size: 2, shuffle: false)
  ...
  test "run runs one iteration with checkpoint params and returns updated flat params" do
    ...
    batch_labels = Nx.select(Nx.less(batch_labels, 50), batch_labels, Nx.broadcast(-100, {batch, seq_len}))
