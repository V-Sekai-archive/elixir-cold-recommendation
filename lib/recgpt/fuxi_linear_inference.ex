defmodule RecGPT.FuxiLinearInference do
  ...
  **Memory: fixed-size state, not a growing KV cache.** FuXi-Linear uses linear attention / retention:
  the recurrence S_t = decay·S_{t-1} + k^T v collapses history into a fixed-size state (per block).
  So memory stays constant with sequence length. The "cache" returned here is that recurrent state
  plus, for Channel T/P, a growing history used to compute one new row.

  **Channel P (LinearPositionalChannel)** injects **position-in-sequence** (content-independent).
  attn = emb @ emb.T / dim with sinusoidal emb, then out = attn @ v. **Incremental cost:** we cache
  channel_p_v per block [O(seq_len) memory]; each step we compute only the last row (emb_last @ emb.T) @ v
  in O(seq_len) instead of building the full seq_len×seq_len matrix.

  **Channel T** uses a recurrence S_t = decay(interval_t)·S_{t-1} + k_t^T v_t (Q/K from timestamps),
  so incremental decode is O(1) per step. **Channel P** uses attn = emb @ emb.T (sinusoidal position
  kernel); that kernel is not exponential in (t−j), so incremental decode keeps O(seq_len) cached v.

  **Incremental scaling (per new token):**
  - Memory: O(seq_len) — Retention and Channel T state are O(1); Channel P caches v per block.
  - Compute: Retention O(1); Channel T O(1) via recurrence; Channel P O(seq_len) (last row only).
  """
  ...
  @doc """
  Full forward and return incremental state for autoregressive decode.
  Returns {logits, fuxi_cache}. The "cache" is a fixed-size retention state (recurrent S) per block
  plus Channel T/P history; use forward_incremental/5 with it for O(1) Retention steps per new token.
  """
  @spec forward_with_cache(Nx.Tensor.t(), Nx.Tensor.t(), Nx.Tensor.t(), map()) ::
          {Nx.Tensor.t(), RecGPT.FuxiLinearInference.fuxi_cache()}
  def forward_with_cache(batch_token_ids, batch_aux, embed_mask, params) do
    ...
    {_batch, seq_len, _} = Nx.shape(hidden)
    last_idx = seq_len - 1
    last_hidden = Nx.slice_along_axis(hidden, last_idx, 1, axis: 1) |> Nx.squeeze(axes: [1])
  ...
  @doc """
  Incremental forward for one new token using saved state. batch_token_ids must be (batch, 1).
  Returns {logits, new_cache}. Uses fixed-size retention state (no growing KV cache). Pass nil or []
  as past_key_values to run full forward (no saved state).
  """
  @spec forward_incremental(
          ...
          Nx.Tensor.t(),
          map(),
          list() | RecGPT.FuxiLinearInference.fuxi_cache()
        ) :: {Nx.Tensor.t(), RecGPT.FuxiLinearInference.fuxi_cache()}
  def forward_incremental(batch_token_ids, batch_aux, embed_mask, params, past_key_values) do
    cache = normalize_cache(past_key_values)
    if cache == nil do
      logits = forward(batch_token_ids, batch_aux, embed_mask, params)
      {logits, nil}
    else
      {last_hidden, new_cache} =
        forward_hidden_incremental(batch_token_ids, batch_aux, embed_mask, params, cache)
      {apply_head(last_hidden, params), new_cache}
    end
  end

  # Per block: retention_s is fixed-size recurrent state; channel_*_v/ts grow with seq_len.
  @type fuxi_cache() :: %{
          optional(any()) => any(),
          seq_len: non_neg_integer(),
          blocks: [%{optional(any()) => any()}]
        } | nil

  defp normalize_cache([]), do: nil
  defp normalize_cache(nil), do: nil
  defp normalize_cache(%{blocks: _} = c), do: c
  defp normalize_cache(_), do: nil
  ...
  defp forward_hidden_with_cache(batch_token_ids, batch_aux, embed_mask, params, opts) do
    wte = get_wte(params)
    {batch, seq_len} = Nx.shape(batch_token_ids)
    all_timestamps = Keyword.get(opts, :all_timestamps) || position_timestamps(batch, seq_len)
    ...
    {h, blocks_cache} =
      for i <- 0..(n_blocks - 1), reduce: {x, []} do
        {acc, acc_cache} ->
          {out, block_cache} =
            fuxi_block_with_cache(acc, i, seq_len, all_timestamps, invalid_attn_mask, params)
          {out, acc_cache ++ [block_cache]}
      end

    cache = %{seq_len: seq_len, blocks: blocks_cache}
    {apply_ln_f(h, params), cache}
  end

  defp forward_hidden_incremental(batch_token_ids, batch_aux, embed_mask, params, cache) do
    wte = get_wte(params)
    {batch, 1} = Nx.shape(batch_token_ids)
    seq_len = cache.seq_len + 1

    flat_ids = Nx.reshape(batch_token_ids, {batch * 1})
    token_embeds = Nx.gather(wte, Nx.new_axis(flat_ids, -1))
    token_embeds = Nx.reshape(token_embeds, {batch, 1, @n_embd})
    aux_768 = apply_aux_encoder(batch_aux, embed_mask, params)
    x_new = Nx.add(token_embeds, aux_768)

    # One new position; timestamps: use position index (seq_len - 1)
    new_ts = Nx.broadcast(Nx.tensor(seq_len - 1, type: {:f, 32}), {batch, 1, @channel_t_heads})
    n_blocks = count_blocks(params)

    {last_hidden, new_blocks} =
      Enum.reduce(Enum.zip(0..(n_blocks - 1), cache.blocks), {x_new, []}, fn {i, block_cache},
                                                                              {h_in, new_acc} ->
        {out_last, new_block} =
          fuxi_block_incremental(h_in, i, seq_len, new_ts, params, block_cache)
        h_next = Nx.new_axis(out_last, 1)
        {h_next, new_acc ++ [new_block]}
      end)

    new_cache = %{seq_len: seq_len, blocks: new_blocks}
    last_hidden = Nx.squeeze(last_hidden, axes: [1])
    {apply_ln_f(last_hidden, params), new_cache}
  end
  ...
  defp fuxi_block_with_cache(hidden, block_idx, seq_len, all_timestamps, invalid_attn_mask, params) do
    base = "fuxi.block.#{block_idx}."
    normed = layer_norm(hidden, params[base <> "ln.weight"], params[base <> "ln.bias"])
    ...
    # Retention with state for cache
    {ret_out, retention_s} =
      retention_forward_with_state(q, k, v, seq_len, invalid_attn_mask, base, params)

    {ct_out, channel_t_s, channel_t_last_ts} =
      channel_t_forward_with_state(normed, seq_len, all_timestamps, invalid_attn_mask, base, params)
    cp_out = channel_p_forward(normed, seq_len, invalid_attn_mask, base, params)

    channel_p_v =
      if params[base <> "channel_p.proj_p.weight"] do
        Nx.dot(normed, [2], params[base <> "channel_p.proj_p.weight"], [0])
      else
        Nx.slice(normed, [0, 0, 0], [elem(Nx.shape(normed), 0), seq_len, @value_dim])
      end

    combined = Nx.concatenate([ret_out, ct_out, cp_out], axis: 2)
    attn_out = Nx.multiply(u, combined)
    mffn_out = mffn_forward(attn_out, hidden, base, params)
    out = Nx.add(hidden, mffn_out)

    block_cache = %{
      retention_s: retention_s,
      channel_t_s: channel_t_s,
      channel_t_last_ts: channel_t_last_ts,
      channel_p_v: channel_p_v
    }

    {out, block_cache}
  end

  defp fuxi_block_incremental(hidden, block_idx, seq_len, new_timestamps, params, block_cache) do
    base = "fuxi.block.#{block_idx}."
    normed = layer_norm(hidden, params[base <> "ln.weight"], params[base <> "ln.bias"])
    ...
    # Retention: incremental with cached state
    {ret_out, retention_s_new} =
      retention_forward_incremental(block_cache.retention_s, q, k, v, base, params)

    # Channel T: O(1) recurrence; Channel P: last row only from cached v
    v_t_new = Nx.dot(normed, [2], params[base <> "channel_t.proj_v.weight"], [0])
    {ct_out, channel_t_s_new} =
      channel_t_forward_incremental(
        block_cache.channel_t_s,
        block_cache.channel_t_last_ts,
        new_timestamps,
        v_t_new,
        base,
        params
      )
    cp_out = channel_p_forward_one_row(normed, seq_len, block_cache, base, params)

    channel_p_v_new =
      if params[base <> "channel_p.proj_p.weight"] do
        Nx.dot(normed, [2], params[base <> "channel_p.proj_p.weight"], [0])
      else
        Nx.slice(normed, [0, 0, 0], [elem(Nx.shape(normed), 0), 1, @value_dim])
      end
    ...
    mffn_out = mffn_forward(attn_out, hidden, base, params)
    out = Nx.add(hidden, mffn_out)
    out_last = Nx.squeeze(out, axes: [1])

    new_block_cache = %{
      retention_s: retention_s_new,
      channel_t_s: channel_t_s_new,
      channel_t_last_ts: new_timestamps,
      channel_p_v: Nx.concatenate([block_cache.channel_p_v, channel_p_v_new], axis: 1)
    }

    {out_last, new_block_cache}
  end

  # Incremental: only need last row of (emb @ emb.T / dim) @ v = (emb_last @ emb.T / dim) @ v_all.
  # Compute as (1, dim) @ (dim, seq_len) -> (1, seq_len) then (1, seq_len) @ (seq_len, d) -> O(seq_len), no seq_len² matrix.
  defp channel_p_forward_one_row(normed_x, seq_len, block_cache, base, params) do
    {batch, _n, _} = Nx.shape(normed_x)
    proj_w = params[base <> "channel_p.proj_p.weight"]
    v_new = if proj_w, do: Nx.dot(normed_x, [2], proj_w, [0]), else: Nx.slice(normed_x, [0, 0, 0], [batch, 1, @value_dim])
    v_all = Nx.concatenate([block_cache.channel_p_v, v_new], axis: 1)
    ...
    emb_last = Nx.slice_along_axis(emb, seq_len - 1, 1, axis: 0)
    # row_weights (1, seq_len) = emb_last (1, dim) @ emb.T (dim, seq_len); causal: last row is all j in [0, seq_len-1] so mask is ones
    row_weights = Nx.dot(emb_last, [1], emb, [1])
    row_weights = Nx.divide(row_weights, max(div(@channel_p_dim, 2), 1))
    # (1, seq_len) @ (batch, seq_len, value_dim) -> contract 1; result (1, batch, value_dim) or when batch=1 sometimes (1, value_dim)
    last_row = Nx.dot(row_weights, [1], [0], v_all, [1], [0])
    last_row =
      if Nx.rank(last_row) == 2 do
        Nx.new_axis(last_row, 1)
      else
        Nx.transpose(last_row, axes: [1, 0, 2])
      end
    alpha = params[base <> "channel_p.alpha"]
    beta = params[base <> "channel_p.beta"]
    if alpha && beta do
      v_last = Nx.slice_along_axis(v_all, seq_len - 1, 1, axis: 1)
      Nx.add(Nx.multiply(last_row, alpha), Nx.multiply(v_last, beta))
    else
      last_row
    end
  end
  ...
  defp retention_forward_incremental(s_old, q, k, v, base, params) do
    # q,k,v from block are (batch, 1, value_dim); reshape to (batch, 1, n_head, head_dim)
    {batch, _one, _} = Nx.shape(q)
    q = Nx.reshape(q, {batch, 1, @n_head, @head_dim})
    k = Nx.reshape(k, {batch, 1, @n_head, @head_dim})
    v = Nx.reshape(v, {batch, 1, @n_head, @head_dim})
    decay = retention_decay_per_step(base, params)
    decay_bc = Nx.reshape(decay, {1, @n_head, 1, 1})
    k_t = Nx.squeeze(k, axes: [1])
    v_t = Nx.squeeze(v, axes: [1])
    q_t = Nx.squeeze(q, axes: [1])
    k_t = Nx.new_axis(k_t, -1)
    v_t = Nx.new_axis(v_t, 2)
    kv = Nx.multiply(k_t, v_t)
    s_new = Nx.add(Nx.multiply(decay_bc, s_old), kv)
    q_flat = Nx.reshape(q_t, {batch * @n_head, 1, @head_dim})
    s_flat = Nx.reshape(s_new, {batch * @n_head, @head_dim, @head_dim})
    out_flat = Nx.dot(q_flat, [2], [0], s_flat, [1], [0])
    out_t = Nx.reshape(out_flat, {batch, @n_head, @head_dim})
    out = Nx.reshape(out_t, {elem(Nx.shape(q_t), 0), 1, @value_dim})
    out = layer_norm(out, params[base <> "retention.ln.weight"], params[base <> "retention.ln.bias"])
    {out, s_new}
  end
  ...
  # Channel T recurrent forward: same output as channel_t_forward but computed step-by-step; returns
  # {out, s_final, last_ts} for cache. S_t = decay_step_t * S_{t-1} + k_t^T v_t, out_t = q_t @ S_t.
  defp channel_t_forward_with_state(normed_x, seq_len, all_timestamps, _invalid_attn_mask, base, params) do
    {batch, _n, _} = Nx.shape(normed_x)
    proj_w = params[base <> "channel_t.proj_v.weight"]
    v = Nx.dot(normed_x, [2], proj_w, [0])
    ...
    if seq_len < 2 do
      alpha = params[base <> "channel_t.alpha"]
      beta = params[base <> "channel_t.beta"]
      out = if alpha && beta, do: Nx.add(Nx.multiply(v, alpha), Nx.multiply(v, beta)), else: v
      # No recurrence state for seq_len 1; last_ts for next step
      last_ts = all_timestamps
      s_final = channel_t_zero_state(batch, params)
      {out, s_final, last_ts}
    else
      base_val = 2
      idx = Nx.iota({@channel_t_heads}, type: {:s, 64})
      ...
      {s_final, out_rows} =
        Enum.reduce(0..(seq_len - 1), {channel_t_zero_state(batch, params), []}, fn t, {s_old, acc} ->
          q_t = Nx.slice(q, [0, t, 0], [batch, 1, @channel_t_heads * 4])
          k_t = Nx.slice(k, [0, t, 0], [batch, 1, @channel_t_heads * 4])
          v_t = Nx.slice(v_4d, [0, t, 0, 0], [batch, 1, @channel_t_heads * 4, v_per_head])

          # Decay from position t-1 to t is exp(-log_decay_pos[t-1]) (cumsum alignment in ext_decay_attn_map)
          decay_step =
            if t == 0 do
              Nx.broadcast(Nx.tensor(1.0, type: {:f, 32}), {batch, @channel_t_heads * 4, 1})
            else
              ld = Nx.slice(log_decay_pos, [0, t - 1, 0], [batch, 1, 16])
              ld = Nx.reshape(ld, {batch, 16, 1}) |> Nx.broadcast({batch, 16, 2}) |> Nx.reshape({batch, @channel_t_heads * 4})
              Nx.reshape(Nx.exp(Nx.negate(ld)), {batch, @channel_t_heads * 4, 1})
            end

          # k_t (batch, 1, 32), v_t (batch, 1, 32, 4): add trailing axis so multiply broadcasts to (batch, 1, 32, 4)
          kv = Nx.multiply(Nx.new_axis(k_t, -1), v_t)
          kv = Nx.squeeze(kv, axes: [1])
          s_new = Nx.add(Nx.multiply(decay_step, s_old), kv)
          # q_t (batch, 1, 32) * s_new (batch, 32, 4) -> reshape q_t to (batch, 32, 1) for broadcast -> (batch, 32, 4)
          q_t_2d = Nx.squeeze(q_t, axes: [1])
          out_t = Nx.multiply(Nx.new_axis(q_t_2d, -1), s_new)
          # Insert seq axis at 1 (no name) so stack/concatenate does not conflict
          out_t = Nx.new_axis(out_t, 1)
          {s_new, [out_t | acc]}
        end)

      # Stack (batch, 1, 32, 4) tensors -> (batch, seq_len, 1, 32, 4), squeeze -> (batch, seq_len, 32, 4)
      out = out_rows |> Enum.reverse() |> Nx.stack(axis: 1) |> Nx.squeeze(axes: [2])
      out = Nx.reshape(out, {batch, seq_len, @value_dim})
      ...
      last_ts = Nx.slice(all_timestamps, [0, seq_len - 1, 0], [batch, 1, @channel_t_heads])
      {out, s_final, last_ts}
    end
  end

  defp channel_t_zero_state(batch, _params) do
    v_per_head = div(@value_dim, @channel_t_heads * 4)
    Nx.broadcast(Nx.tensor(0.0, type: {:f, 32}), {batch, @channel_t_heads * 4, v_per_head})
  end

  # One-step Channel T update for incremental decode: S_new = decay_step * S_old + k_new * v_new, out = q_new @ S_new.
  # q_new from last_ts (shifted), k_new from new_ts; decay_step from interval (new_ts - last_ts).
  defp channel_t_forward_incremental(s_old, last_ts, new_ts, v_new, base, params) do
    {batch, _n, _} = Nx.shape(v_new)
    base_val = 2
    idx = Nx.iota({@channel_t_heads}, type: {:s, 64})
    intervals = Nx.pow(base_val, idx)
    scale_factor = Nx.multiply(2 * :math.pi(), Nx.pow(1 / base_val, Nx.as_type(idx, {:f, 32})))
    ...
    theta_new = Nx.multiply(
      Nx.remainder(new_ts, Nx.reshape(intervals, {1, 1, @channel_t_heads})),
      Nx.reshape(scale_factor, {1, 1, @channel_t_heads})
    )
    cos_new = Nx.cos(theta_new)
    sin_new = Nx.sin(theta_new)
    # Use theta_new for q_new so last-position output matches full forward (q at last position uses current theta there)
    q_new = Nx.concatenate([sin_new, Nx.negate(cos_new), cos_new, sin_new], axis: 2)
    k_new = Nx.concatenate([cos_new, cos_new, sin_new, sin_new], axis: 2)

    interval = Nx.clip(Nx.subtract(new_ts, last_ts), 0, 1.0e9)
    hinterval = Nx.multiply(interval, Nx.reshape(scale_factor, {1, 1, @channel_t_heads}))
    log_decay = Nx.multiply(hinterval, Nx.negate(Nx.log(gamma)))
    log_decay = Nx.concatenate([log_decay, log_decay], axis: 2)
    # Expand 16 -> 32 to match channel_t_heads * 4 (same as full forward)
    decay_step = Nx.exp(Nx.negate(log_decay))
    decay_step = decay_step |> Nx.reshape({batch, 16, 1}) |> Nx.broadcast({batch, 16, 2}) |> Nx.reshape({batch, @channel_t_heads * 4, 1})

    v_per_head = div(@value_dim, @channel_t_heads * 4)
    v_4d = Nx.reshape(v_new, {batch, 1, @channel_t_heads * 4, v_per_head})
    kv = Nx.multiply(Nx.new_axis(k_new, -1), v_4d)
    kv = Nx.squeeze(kv, axes: [1])
    s_new = Nx.add(Nx.multiply(decay_step, s_old), kv)
    # q_new (batch, 1, 32) * s_new (batch, 32, 4) -> broadcast via (batch, 32, 1) * (batch, 32, 4)
    q_2d = Nx.squeeze(q_new, axes: [1])
    out = Nx.multiply(Nx.new_axis(q_2d, -1), s_new)
    out = Nx.reshape(out, {batch, 1, @value_dim})

    alpha = params[base <> "channel_t.alpha"]
    beta = params[base <> "channel_t.beta"]
    out =
      if alpha && beta do
        Nx.add(Nx.multiply(out, alpha), Nx.multiply(v_new, beta))
      else
        out
      end
