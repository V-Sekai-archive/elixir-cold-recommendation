defmodule RecGPT.CheckpointExport do
  @moduledoc """
  Write RecGPT checkpoint to an export directory (manifest.json + .npy files)
  compatible with `RecGPT.CheckpointLoader.load_from_export/1`.

  Uses the `:npy` package to save each tensor. Use this to:
  - Save trained params from `RecGPT.AxonTrain.run/3` to disk.
  ...
  Keys are preserved in the manifest so `CheckpointLoader.load_from_export/1` returns
