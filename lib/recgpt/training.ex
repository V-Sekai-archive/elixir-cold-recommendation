defmodule RecGPT.Training do
  @moduledoc """
  RecGPT training: data batching and loss. build_train_batch/5, encode_aux/3, loss_shifted_ce/2.
  When timestamps are provided, builds all_timestamps (batch, seq_len, 8) with cumulative ms
  from sequence start for FuXi-Linear Temporal Retention Channel.
  """
  ...
  def build_train_batch(seqs, token_id_list, item_embeddings, batch_indices, timestamps \\ nil) do
    ...
    {batch_seq, batch_labels, batch_aux_list, embed_mask_list, timestamp_rows} =
      Enum.reduce(
        ...
        fn {seq, ts}, {seq_acc, label_acc, aux_acc, mask_acc, ts_acc} ->
          ...
          token_list =
            for item_id <- seq, do: Enum.at(token_id_list, item_id) || List.duplicate(0, 4)
    ...
    all_timestamps =
      if Enum.all?(timestamp_rows, & &1) do
        ...
        # (batch, seq_len) -> (batch, seq_len, 1) -> broadcast to (batch, seq_len, 8)
  ...
  @doc """
  Multi-Token Prediction loss over the last 4 positions (next item's 4 tokens).

  Penalizes errors across all 4 tokens of the next item in the same training step.
  Use with loss_shifted_ce: total = loss_shifted_ce + mtp_weight * loss_mtp_last_4.
  When seq_len < 4 returns 0.0. Defn-compatible.
  """
  def loss_mtp_last_4(logits, labels) do
    ...
    if seq_len < 4 do
      ...
      start_idx = seq_len - 4
      logits_last = Nx.slice_along_axis(logits, start_idx, 4, axis: 1)
      labels_last = Nx.slice_along_axis(labels, start_idx, 4, axis: 1)
      {batch, _four, _} = Nx.shape(logits_last)
      logits_flat = Nx.reshape(logits_last, {batch * 4, vocab})
      labels_flat = Nx.reshape(labels_last, {batch * 4})
      valid = Nx.not_equal(labels_flat, @label_ignore)
      exp = Nx.exp(logits_flat)
      probs = Nx.divide(exp, Nx.sum(exp, axes: [1]) |> Nx.new_axis(-1))
      log_probs = Nx.log(Nx.add(probs, 1.0e-6))
