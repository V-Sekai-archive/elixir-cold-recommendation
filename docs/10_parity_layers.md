...
Per-layer parity with Python RecGPT must be tracked and validated. Without task lists and validation steps per layer, gaps and regressions go unnoticed.
...
| Match Python MPNet (`normalize_embeddings=False`) | âœ… Validated | Same model id; `embedding_processor: nil` (no L2). Parity via unit tests.              |
...
| Task                                                      | Status   | Notes                                                                                                |
| --------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------- |
| FSQ levels [8,8,8,6,5], basis, bound, round_ste, quantize | âœ… Done | `RecGPT.FSQ` port of RecGPT utils/fsq.py.                                                            |
...
| **FSQ encode: 768-d â†’ 4 token IDs**                     | âœ… Done | `RecGPT.FSQEncoder.encode_embeddings_to_token_id_list/3`; matches Python `make_token_list.py` logic. |
| **Python vs Elixir FSQ parity**                           | âœ… Done | Parity via unit tests and pipeline integration tests.                                                |
...
| Same batch shape / format as Python RecGPT                       | â†’ Verified | Matches HKUDS/RecGPT `utils/data.py` `GPT2RecBatchTrainAuxData`: max_length=256, padding_id=15360, seq_token_capacity=1024, label_ignore=-100, right-padding; aux (256 items â†’ 1024�192) and mask (1024�1) match. |
...
| Flow                                                                             | Status       | Notes                                                                                                                                                               |
| -------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Data only:** item_text_dict â†’ embeddings â†’ token_id_list â†’ train batches | âœ… Done     | No Python needed: Embedding + FSQEncoder + Training.                                                                                                                |
| **Train (Python):** pre_train.py with our token_id_list + embeddings             | â†’ Possible | Build token_id_list in Elixir; export or point Python at same data.                                                                                                 |
...
| **Zero-shot eval (predict.py) with our data**                                    | â†’ Possible | Python script; we can produce compatible pkl/npy from Elixir pipeline.                                                                                              |
...
| FSQ + FSQEncoder                               | â†’ Implemented, unit tested | Same logic as Python.                                                        |
