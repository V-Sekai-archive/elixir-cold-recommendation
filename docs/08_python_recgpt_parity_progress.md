# Proposal: Python RecGPT parity progress

Sub-proposal of the [documentation index](README.md). Task list for how close the **recgpt** Elixir package is to matching the [Python RecGPT](https://github.com/HKUDS/RecGPT) pipeline and model.
...
Maintain a **parity progress** doc: at-a-glance table, Python ↔ Elixir mapping, and per-layer task lists with status and notes. Parity is validated via unit and integration tests only; no Python in this codebase. Reference: [RecGPT paper](https://arxiv.org/abs/2506.06270), [hkuds/RecGPT_model](https://huggingface.co/hkuds/RecGPT_model).
...
| Task                                                    | Status  | Notes                                                                                                                                                         |
| ------------------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Checkpoint loader**                                   | ✅ Done | `RecGPT.CheckpointLoader.load_from_export/1`; reads manifest.json + .npy from export dir (see [07_recgpt_checkpoint_layout](07_recgpt_checkpoint_layout.md)). |
...
**Checkpoint key compatibility:** Inference accepts RecGPT export keys: `gpt2model.wte.weight` (sliced to 15361 rows when checkpoint has full GPT-2 vocab), `gpt2model.wpe.weight`, `gpt2model.h.{i}.*`, `linear_layer.weight`/`norm_aux.*` for aux encoder, `pred_head.weight`/`pred_head.bias` for head. See [07_recgpt_checkpoint_layout](07_recgpt_checkpoint_layout.md).
...
- **1. Text → embeddings** (above) — Embedding, encode_item_text_dict, parity.
- **2. FSQ** (above) — FSQ, FSQEncoder, levels, load params, encode.
- **3. Training data and loss** (above) — build_train_batch, encode_aux, loss_shifted_ce.
- **4. Model forward** (above) — CheckpointLoader, Inference.forward.
- **5. Decoding and catalog** (above) — Trie, Decode.beam_search.
- **6. Checkpoint and export** (above) — export_ckpt, load_from_export.
- **7. End-to-end flows** (above) — Data pipeline, inference, serve.
...
| Doc                                                                                                               | Description                                                         |
| ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| [03 RecGPT library](03_recgpt_library.md)                                                                         | Modules, deps, tests, training flow.                                |
| [07 RecGPT checkpoint layout](07_recgpt_checkpoint_layout.md)                                                     | state_dict layout, export via mix recgpt.export_ckpt, loader usage. |
...
- [07 Checkpoint layout](07_recgpt_checkpoint_layout.md) — Export and loader.
