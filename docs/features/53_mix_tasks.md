...
| `mix recgpt.convert_trajectories` | Convert MovieLens/KuaiRand/Jon-Becker to canonical JSON. `--from`, `--out`, `--format`. Jon-Becker (Phase 1): requires `--sync-to-db`; see [93](93_pretraining_plan.md). `--no-fetch-titles` skips Polymarket API (JCS placeholder). `--max-api-requests`, `--api-delay-ms`. See [92](92_polymarket_semantic_source.md). |
...
Paths default to `data/steam/` and `data/fuxi_ckpt_export` (FuXi-Linear). Env: `RECGPT_FIXTURE`, `RECGPT_CKPT_EXPORT` for serve.
