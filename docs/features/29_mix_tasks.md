...
| `mix recgpt.convert_trajectories` | Convert KuaiRand (default) or MovieLens to canonical JSON. `--from`, `--out`, `--format`. See [43](43_pretraining_plan.md). |
...
| `mix recgpt.pretrain`      | Pretrain on train sequences with fixture + checkpoint; write to `--out`. File-based: `--train`, `--items` paths; DB: `--train db --items db`. See [43](43_pretraining_plan.md). `--epochs 5`, `--eval-test-every N --test PATH` for train/test loss. |
