...
- **RecGPT artifacts:** Fixture, checkpoint, train data are read/written as bulk blobs; one GET/PUT per artifact. waffle_ecto fits when you persist **metadata** in Ecto and the blob in local or object store. See [13 Infrastructure and serving](13_infrastructure_serving.md#catalog-storage-object-store-semantics).
- **Artifact catalogue:** `RecGPT.Catalog.Artifact` has an optional `path` field for existing pipeline paths. The migration seeds the table with the default artifact kinds (fixture, checkpoint, train_sequences, cold_train_sequences, test_sequences, cold_test_sequences, items) and their default paths. Use `Artifact.default_artifact_kinds/0` for the list; query the table for stored paths.
- **Path resolution:** All artifact handling uses the catalogue when opts and env are unset. `Artifact.resolve_path(name)` returns the stored path for a named artifact (or nil). Mix tasks (`recgpt.serve`, `recgpt.eval`, `recgpt.pretrain`, `recgpt.build_fixture`, `recgpt.trace_predict`, `recgpt.eval_grpc`, `recgpt.ad_hoc_test`) and `RecGPT.ReleaseTasks.serve/0` resolve fixture, checkpoint, train, test, and items paths in this order: **CLI opts → env vars → catalogue (waffle_ecto) → default path**.
