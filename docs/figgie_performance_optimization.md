...
In Figgie and Polymarket, the "Items" (Suits/Contracts) don't change text every millisecond.
* **Logic:** Compute the 4-token SId **once** when a market opens.
...
*Note: If you want to dive deeper into the specific Elixir `Nx` profiler outputs to see exactly which line is causing the 90ms spike, we can run a `Benchee` suite next.*